...
export function send_message_success(
    ...
    if (sent_message.type === "stream") {
        suggest_topic_title(sent_message.stream_id, sent_message.topic, data.id);
    }
}

// Ask the server whether the conversation has drifted away from its topic
// title.  This runs after the message has already been delivered, so the
// model round trip never delays or blocks sending, and a failure here is
// silently ignored (channel.post ignores errors by default) rather than
// interrupting the user with an error they cannot act on.
function suggest_topic_title(stream_id: number, topic_name: string, message_id: number): void {
    void channel.post({
        url: "/json/topics/improve-title",
        data: {stream_id, topic_name},
        success(raw_data: unknown) {
            ...
            if (!data.drifted || suggested_title === null) {
                return;
            }
            // Renaming a topic affects everyone in the channel, so the
            // suggestion is never applied automatically; the user has to
            // accept it explicitly.
            dialog_widget.launch({
                modal_title_text: $t({defaultMessage: "Suggested topic title"}),
                modal_content_html: render_topic_title_suggestion({
                    current_title: topic_name,
                    suggested_title,
                    reason: data.reason,
                }),
                close_on_submit: true,
                modal_submit_button_text: $t({defaultMessage: "Rename topic"}),
                modal_exit_button_text: $t({defaultMessage: "Keep current title"}),
                on_click() {
                    void channel.patch({
                        url: `/json/messages/${data.message_id ?? message_id}`,
                        data: {
