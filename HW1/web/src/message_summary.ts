import {$} from "jquery";
import * as z from "zod/mini";

import render_topic_summary from "../templates/topic_summary.hbs";
import render_unread_message_recap from "../templates/unread_message_recap.hbs";

import * as channel from "./channel.ts";
import * as dialog_widget from "./dialog_widget.ts";
import {Filter} from "./filter.ts";
import {$t} from "./i18n.ts";
import * as loading from "./loading.ts";
import * as message_fetch from "./message_fetch.ts";
import * as rendered_markdown from "./rendered_markdown.ts";
import * as unread from "./unread.ts";
import * as unread_ops from "./unread_ops.ts";
import * as util from "./util.ts";

export function get_narrow_summary(channel_id: number, topic_name: string): void {
    const filter = new Filter([
        {operator: "channel", operand: `${channel_id}`},
        {operator: "topic", operand: topic_name},
    ]);
    const data = {narrow: message_fetch.get_narrow_for_message_fetch(filter)};
    const display_topic_name = util.get_final_topic_display_name(topic_name);
    const unread_topic_params = {
        modal_submit_button_text: $t({defaultMessage: "Mark topic as read"}),
        modal_exit_button_text: $t({defaultMessage: "Close"}),
        on_click() {
            unread_ops.mark_topic_as_read(channel_id, topic_name);
        },
        single_footer_button: false,
    };

    let params = {
        modal_submit_button_text: $t({defaultMessage: "Close"}),
        on_click() {
            // Just close the modal, there is nothing else to do.
        },
        single_footer_button: true,
    };
    if (unread.topic_has_any_unread(channel_id, topic_name)) {
        params = {
            ...params,
            ...unread_topic_params,
        };
    }
    dialog_widget.launch({
        modal_title_text: display_topic_name,
        modal_content_html: "<div></div>", // TODO: Add a loading indicator here instead of a placeholder.
        close_on_submit: true,
        id: "topic-summary-modal",
        footer_minor_text: $t({defaultMessage: "AI summaries may have errors."}),
        ...params,
        on_show() {
            $("#topic-summary-modal .modal__content").addClass("hide");
        },
        post_render() {
            const close_on_success = false;
            dialog_widget.submit_api_request(
                channel.get,
                "/json/messages/summary",
                data,
                {
                    success_continuation(response_data) {
                        const data = z.object({summary: z.string()}).parse(response_data);
                        const summary_markdown = data.summary;
                        const summary_html = render_topic_summary({
                            summary_markdown,
                        });
                        $("#topic-summary-modal .modal__content")
                            .removeClass("hide")
                            .addClass("rendered_markdown");
                        $("#topic-summary-modal .modal__content").html(summary_html);
                        rendered_markdown.update_elements(
                            $("#topic-summary-modal .modal__content"),
                        );
                    },
                },
                close_on_success,
            );
        },
    });
}

export function get_unread_message_recap(): void {
    dialog_widget.launch({
        modal_title_text: $t({defaultMessage: "Unread message recap"}),
        modal_content_html: '<div id="unread-message-recap-loading"></div>',
        close_on_submit: false,
        id: "unread-message-recap-modal",
        footer_minor_text: $t({defaultMessage: "AI-generated content may have errors."}),
        single_footer_button: true,
        modal_submit_button_text: $t({defaultMessage: "Close"}),
        on_click() {
            // Nothing to submit; the button just closes the modal.
        },
        post_render() {
            const $content = $("#unread-message-recap-modal .modal__content");
            // The recap requires a round trip to the model provider, which
            // takes a few seconds, so show a spinner rather than an empty
            // modal while we wait.  The indicator sizes its own container,
            // so it lives in a dedicated element that we replace afterwards.
            const $loading = $("#unread-message-recap-loading");
            loading.make_indicator($loading, {
                text: $t({defaultMessage: "Summarizing your unread messages…"}),
            });

            channel.get({
                url: "/json/messages/recap",
                success(data: unknown) {
                    const recap = z
                        .object({
                            summary: z.string(),
                            references: z.array(
                                z.object({
                                    message_id: z.number(),
                                    sender: z.string(),
                                    topic: z.string(),
                                    link: z.string(),
                                }),
                            ),
                            truncated: z.boolean(),
                        })
                        .parse(data);
                    loading.destroy_indicator($loading);
                    $content.html(
                        render_unread_message_recap({
                            ...recap,
                            has_references: recap.references.length > 0,
                        }),
                    );
                    // The summary is server-rendered Markdown, so it gets the
                    // same post-processing as ordinary message content.
                    rendered_markdown.update_elements($content.find(".unread-recap-summary"));
                    // Clicking a reference should narrow to the message and
                    // leave the modal, like any other in-app link.
                    $content.find("a").on("click", () => {
                        dialog_widget.close();
                    });
                },
                error(xhr) {
                    loading.destroy_indicator($loading);
                    $content.text(
                        channel.xhr_error_message(
                            $t({defaultMessage: "Could not generate a recap"}),
                            xhr,
                        ),
                    );
                },
            });
        },
    });
}
